...
class Clipboard(object):
    ...
    def get_data(self, format):
        ...
        if pointer == c_void_p(0) and get_last_error() != 0:
            raise WindowsError()
        try:
            CopyMemory = kernel32.RtlCopyMemory
            CopyMemory(data, handle, size)
        finally:
    ...
    def set_data(self, format, data):
        ...
        try:
            ...
            try:
                CopyMemory = kernel32.RtlCopyMemory
                CopyMemory(pointer, data, len(data))
            finally:
                GlobalUnlock(handle)
    ...
    def register_format(self, format_name):
        user32 = windll.user32
        RegisterClipboardFormat = user32.RegisterClipboardFormat
        return RegisterClipboardFormat(format_name).raw
...
if __name__ == '__main__':
    import sys
    print(sys.argv[-1], file = open('hoge.txt', 'w'))
    with Clipboard() as clipboard:
        clipboard.set_unicode_text(sys.argv[-1])
